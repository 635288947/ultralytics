...
def test_rtdetr(task="detect", model="yolov8n-rtdetr.yaml", data="coco8.yaml"):
    ...
    run(f"yolo predict {task} model={model} source={ASSETS / 'test.jpg'} imgsz=160 save save_crop save_txt")
    if TORCH_1_9:
        run(f"yolo predict {task} model='rtdetr-l.pt' source={ASSETS / 'test.jpg'} imgsz=160 save save_crop save_txt")
